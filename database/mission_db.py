...
class MissionDB:
    ...
    def assign_mission(m_id, a_id):
        with DBConnection.get_connection() as conn:
            ...
            if (
                mission["risk_level"] == "CRITICAL"
                and agent["agent_rank"] != "Commander"
            ):
                raise ValueError(
                    f"the mission level is critical, you need agent with rank Commander fro this"
                )

            status_mission = mission["status"]
            if status_mission.upper() != "NEW":
                raise ValueError(
                    f"field, you can't assign mission with other status, only NEW"
                )
    ...
    def update_mission_status(id, status):
        ...
        if status == "ASSIGNED":
            if current_status != "NEW":
                raise ValueError(
                    f"It is not possible to associate a task with a status other than New"
                )

        if status == "IN_PROGRESS":
            if current_status != "ASSIGNED":
                raise ValueError(
                    "You cannot start a task with a status other than ASSIGNED."
                )

        if status == "CANCELLED":
            if current_status.upper() != "NEW" and current_status.upper() != "ASSIGNED":
                raise ValueError(
                    "You can only cancel a task if the status is NEW or ASSIGNED ."
                )

        if status == "FAILED" or status == "COMPLETED":
            if current_status != "IN_PROGRESS":
                raise ValueError(
                    "Only a task with a status of IN_PROGRESS can be finish."
                )
    ...
    def count_open_missions():
        with DBConnection.get_connection() as conn:
            with conn.cursor(dictionary=True) as cursor:
                sql = """SELECT COUNT(*) AS open_missions FROM missions WHERE status in('IN_PROGRESS', 'ASSIGNED', 'NEW')"""
                cursor.execute(sql)
