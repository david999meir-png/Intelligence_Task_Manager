...
def update_agent(id: int, agent: AgentUP):
    ...
    if len(agent.model_dump(exclude_none=True)) == 0:
        logger.error(f"try to update an agent with empty data: {id}")
        raise HTTPException(
            status_code=400, detail=f"try to update an agent with empty data: {id}"
        )
...
def deactive_agent(id: int):
    ...
    if not changed:
        ...
        raise HTTPException(
            status_code=400, detail=f"diactive field: {id} alrady deactive"
        )
